...
| `CHAT_ENDPOINT_API_KEY` | unset | Optional shared secret for the developer's endpoints. When **set**, every outbound request to `CHAT_ENDPOINT_URL`, `COMMENT_ENDPOINT_URL`, and `USER_LOOKUP_URL` carries it as the `X-Social-Api-Key` header, and the **receiving** service enforces it — the transport never validates anything against it. When **unset**, no auth header is added and those three clients send byte-identical requests to the ones they sent before this var existed. Blank/whitespace-only collapses to unset (an empty header would be *rejected* by an enforcing endpoint — a more confusing failure than "no header configured"). Deliberately **no length floor**, unlike `ADMIN_API_TOKEN`: the value is agreed with the receiving service, so failing boot over a short-but-accepted secret would break a working deploy. Worth setting whenever the chat endpoint is internet-facing — inbound webhooks are signature-verified and the admin routes are token-gated, which leaves this outbound hop as the only unauthenticated one. |
