...
- **The agent itself is not running.** The verify scripts bind the same local `PORT` and the same reserved ngrok domain as `npm run dev` / `npm run dev:loop`, so a running agent produces an immediate `EADDRINUSE` (or a tunnel fight). Stop it first — or skip the verify script entirely and test through the running dev loop instead: a DM answered by the echo stub proves token, subscription, routing, and outbound in one step, which is the same coverage the script builds up piecewise.
- **Roles configured (Development mode only).** While the app is unpublished, the personal Facebook / Instagram accounts you'll use to send inbound test messages must have Tester / Admin / Developer role on the Meta App (Settings → App Roles; Instagram keeps a separate Instagram Tester list). Otherwise the messages compose but never deliver, with no error surface. Once the app is Live, any account works — roles stop mattering for delivery (verified live, August 2026).
- **Instagram only**: "Allow access to messages" toggle is ON in the IG mobile app (Settings and activity → Messages and story replies → Message controls → Connected tools). The toggle exists ONLY in the mobile app — instagram.com's settings do not surface it. When OFF, the verify script's inbound step times out with zero error signal — this is the #1 silent killer.
...
- On the official IG mobile app: Settings and activity → Messages and story replies → Message controls → Connected tools → **Allow access to messages = ON** (mobile app only; not on instagram.com).
...
7. **Outbound reply** (skippable) — `POST graph.instagram.com/{userId}/messages` echoing back to `sender.id`. Meta's 200 + `message_id` is the authoritative success signal (no human-confirm prompt). Unlike Messenger, Instagram has **no `message_echoes` webhook field** to subscribe, so the script waits for no echo — though in practice IG delivers echoes of your own outbound anyway, riding the `messages` field with `is_echo: true` (measured August 2026); the Send API response stays the authoritative confirmation.
...
- *`subscribe_instagram_app` fails with HTTP 400 / code 100* — an invalid field in `SUBSCRIBED_FIELDS.instagram`. `message_echoes` is the classic culprit (Messenger-only; not valid for IG). The set this repo subscribes is `messages, messaging_postbacks, messaging_seen, message_reactions, messaging_referral, comments` — all accepted at Standard Access (though `comments` DELIVERY is Advanced-Access-gated; see [Comments](./comments.md)).
