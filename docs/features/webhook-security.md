...
- `createMetaSignatureVerifier(appSecret: string | readonly string[], logger?)` — Express middleware factory. Passes the secret(s) through to `verifyMetaSignature`. On failure it logs a `secretsCount` field alongside `path` / `signaturePresent` / `bodyBytes`.
