...
// Re-export the canonical error type so existing importers of this module
// (e.g. tests/unit/scripts-graph-api.test.ts and the setup scripts) keep
// working unchanged. The class itself now lives in `src/meta/shared/errors.ts`
// — `src/` is the published package and must never import from `scripts/`, so
// shared code lives under `src/` and `scripts/` imports FROM it (one-way).
export { MetaApiError, type MetaApiErrorArgs } from '../../src/meta/shared/errors.js';
